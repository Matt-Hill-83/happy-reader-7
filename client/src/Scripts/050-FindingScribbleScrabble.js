const test100 = {
  title: "Title Goes Here",
  scenes: [
    [
      `{"kat": "Oh no! look at that baby dragon over there."}`,
      `{"liz": "Oh my goodness!  Why is it all white? It looks like the person that drew this dragon didn’t even bother to color it in"}`,
      `{"kat": "Wow Matt... just... wow!  That is so sloppy... just... wow..."}`,
      `{"liz": "It’s that Splatoon game.  Remember when Matt and Charlie used to sit around and color?  Now they just stare at that screen and giggle like monkeys."}`,
      `{"xxx": "   <------------------------>"}`,
      `{"xxx": "   <------------------------>"}`,
      `{"Kat": "His art has really gone down the tubes."}`,
      `{"liz": "Ha ha! I know right!  I mean at our torsos.  What is this?  What am I looking at here?"}`,
      `{"kat": "Ha! Ha! I think he was too lazy to draw our bodies.  He just pasted the body of a different girl on to our heads."}`,
      `{"kat": "Oh wow, I totally see that now."}`,
      `{"xxx": "   <------------------------>"}`,
      `{"xxx": "   <------------------------>"}`,
      `{"liz": "Ha! Ha! We are the stars of this show, and we look like we got cut in half with safety scissors."}`,
      `{"kat": "But hey, at least we didn’t end up like that witch."}`,
      `{"liz": "Oh I know!  With the watermark on her hat?  I try not to stare.  I saw her at the well trying to scrub it out."}`,
      `{"kat": "That mark's not going anywhere.  She's a J-PEG.  She’s gonna need a codec to get that out."}`,
      `{"xxx": "   <------------------------>"}`,
      `{"xxx": "   <------------------------>"}`,
      `{"kat": "But hey, that new sky Matt put in is fresh!"}`,
      `{"liz": "Yeah, and I love that new splash screen!"}`,
      `{"kat": "I think he stole it from Roman Battle Smash 5."}`,
      `{"kat": "Well at least he used the best Roman Battle Smash!"}`,
      `{"xxx": "   <------------------------>"}`,
      `{"xxx": "   <------------------------>"}`,
      `{"liz": "RBS 6 was the better."}`,
      `{"kat": "Oh please!  Are you still mad because all your battle boats burned up in the harbor?"}`,
      `{"liz": "Sigh. Yeah I suppose.  I never should hae built them out of that pitchy timber."}`,
      `{"kat": "The oracle said wait 12 moons.  Respect the oracle."}`,
      `{"xxx": "   <------------------------>"}`,
      `{"xxx": "   <------------------------>"}`,
      `{"liz": "I have an idea.  We can help that dragon.  Do you have your crayons on you?"}`,
      `{"kat": "Um, hello Kat, I’m 6 years old. Of course I have my crayons on me! Oh wait, no, I forgot them. I’m thirsty."}`,
      `{"xxx": "   <------------------------>"}`,
      `{"xxx": "   <------------------------>"}`,
      `{"kat": "Matt! Hello, Matt! For Pete’s sake!  Can we get this story moving please?"}`,
      `{"liz": "Oh look, a box of markers is on the grass. I didn’t see that before."}`,
      `{"kat": "Maybe they are Magic Markers! Ha!"}`,
      `{"liz": "Kat, you crack me up!"}`,
      `{"xxx": "   <------------------------>"}`,
      `{"xxx": "   <------------------------>"}`,
      `{"liz": "Hi ya widdle draggy waggy."}`,
      `{"kat": "Did you lose your mommy?"}`,
      `{"dragon": "Eep... Eep..."}`,
      `{"kat": "The poor little girl."}`,
      `{"liz": "Is it ok if we color you in a little?"}`,
      `{"kat": "Yip! Yip!"}`,
      `{"liz": "Kat, come on, give me a hand."}`,
    ],
  ],
}

export default test100
